...
class QuestionModelTest(TestCase):
    ...
    def test_was_published_recently_with_old_question(self):
        ...
        old_question = Question(publishedDate=time)
        self.assertIs(old_question.wasPublishedRecently(), False)

    def test_was_published_recently_with_recent_question(self):
        time = timezone.now() - datetime.timedelta(hours=23, minutes=59, seconds=59)
        recent_question = Question(publishedDate=time)
...
def create_question(question_text, days):
    ...
    return Question.objects.create(text=question_text, publishedDate=time)
